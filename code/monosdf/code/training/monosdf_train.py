...
class MonoSDFTrainRunner():
    def __init__(self,**kwargs):
        ...
        if scan_id != -1:
            self.expname = self.expname + '/{0}'.format(scan_id) + '/' + self.split
        ...
        self.nepochs = self.max_total_iters // len(self.train_dataset) + 1
    ...
    def run(self):
        ...
        self.iter_step = 0
        tbar = tqdm(total=self.max_total_iters)
        for epoch in range(self.start_epoch, self.nepochs + 1):
            ...
            for data_index, (indices, model_input, ground_truth) in enumerate(self.train_dataloader):
                ...
                if 'jaco_info' in ground_truth:
                    ground_truth['jaco_info'] = ground_truth['jaco_info'].cuda()
                ...
                if self.GPU_INDEX == 0:
                    ...
                    self.writer.add_scalar('Loss/jaco_con', loss_output['jaco_con'].item(), self.iter_step)
                    # print('Jaco Loss : ', loss_output['jaco_con'].item())
        ...
        # mesh.apply_transform(self.train_dataset.scale_mat)
        ...
        mesh.export(os.path.join(self.expdir, self.timestamp, 'result.ply'))
