...
def plot(implicit_network, indices, plot_data, path, epoch, img_res, plot_nimgs, resolution, grid_boundary, level=0, name=None):
...
def get_surface_trace(path, epoch, sdf, resolution=100, grid_boundary=[-2.0, 2.0], return_mesh=False, level=0, name=None):
    ...
    print('Shape z: ', z.shape, f' {np.min(z)} - {np.max(z)}')
    if (not (np.min(z) > level or np.max(z) < level)):
        ...
        print(meshexport)
        if name != None:
            meshexport.export('{0}/{1}'.format(path, name), 'ply')
            print('Export At ', '{0}/{1}'.format(path, name))
        else:
            meshexport.export('{0}/surface_{1}.ply'.format(path, epoch), 'ply')
            print('Export At ', '{0}/surface_{1}.ply'.format(path, epoch))
