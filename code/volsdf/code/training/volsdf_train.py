...
class VolSDFTrainRunner():
    def __init__(self,**kwargs):
        ...
        self.tbar = tqdm(total=self.nepochs)
        self.exps_folder_name = kwargs['exps_folder_name']

        self.expname = self.conf.get_string('train.expname')
        scan_id = kwargs['scan_id'] if kwargs['scan_id'] != -1 else self.conf.get_int('dataset.scan_id', default=-1)
        if scan_id != -1:
            self.expname = os.path.join(self.expname, scan_id)
        ...
        self.nepochs = int(self.nepochs / self.ds_len)
        print(f'Total nEpochs = {self.nepochs}')
    ...
    def run(self):
        ...
        for epoch in range(self.start_epoch, self.nepochs + 1):
            ...
            for data_index, (indices, model_input, ground_truth) in enumerate(self.train_dataloader):
                ...
                # print('Training UV Shape : ', model_input["uv"].shape)
        ...
        print('Train Over, Start Surface extraction')

        self.save_checkpoints(epoch)

        plt.get_surface_trace(path=self.plots_dir,
            epoch=epoch,
