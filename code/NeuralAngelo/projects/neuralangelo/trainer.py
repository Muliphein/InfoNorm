...
from projects.neuralangelo.utils.misc import get_scheduler, eikonal_loss, curvature_loss, jacobi_loss
...
class Trainer(BaseTrainer):
    ...
    def __init__(self, cfg, is_inference=True, seed=0):
        ...
        self.use_jacobi = cfg.trainer.use_jacobi
    ...
    def _compute_loss(self, data, mode=None):
        if mode == "train":
            ...
            if "jaco_con" in self.weights.keys() and self.use_jacobi:
                self.losses["jaco_gra"], self.losses["jaco_con"] = jacobi_loss(data["dnormal_dw"], data["jaco_info"])
                # print(f'Calc Loss  Gra: {self.losses["jaco_gra"]}, Con: {self.losses["jaco_con"]}')
    ...
    @torch.no_grad()
    def calc_psnr(self, cfg):
        # train_data_all = super().test(cfg, self.train_data_loader, mode="val", show_pbar=True)
        super().calc_psnr(self.eval_data_loader, mode="val", show_pbar=True, post_fix="Train")
        # super().calc_psnr(self.train_data_loader, mode="val", show_pbar=True, post_fix="Train")
        super().calc_psnr(self.test_data_loader, mode="val", show_pbar=True, post_fix="Novel")
        # test_data_all = super().test(cfg, self.test_data_loader, mode="val", show_pbar=True)