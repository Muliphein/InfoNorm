...
class Model(BaseModel):
    ...
    def __init__(self, cfg_model, cfg_data):
        ...
        self.use_jacobi = cfg_model.use_jacobi
        # Define models.
    ...
    @torch.no_grad()
    def inference_lite(self, data):
        self.eval()
        # Render the full images.
        output = self.render_image_lite(data["pose"], data["intr"], image_size=self.image_size_val,
                                   stratified=False, sample_idx=data["idx"])  # [B,N,C]
    ...
    def render_image_lite(self, pose, intr, image_size, stratified=False, sample_idx=None):
        ...
        for center, ray, _ in self.ray_generator(pose, intr, image_size, full_image=True):
            ...
            output_batch = self.render_rays_lite(center, ray_unit, sample_idx=sample_idx, stratified=stratified)
    ...
    def render_rays_lite(self, center, ray_unit, sample_idx=None, stratified=False):
        with torch.no_grad():
    ...
    def render_rays(self, center, ray_unit, sample_idx=None, stratified=False):
        ...
        if self.with_background:
            ...
            dists = output_object["dists"]  # [B,R,No,1]
            alphas = output_object["alphas"]  # [B,R,No]
            dnormal_dws = output_object["dnormal_dw"] # [B,R,No,3*256]
        weights = render.alpha_compositing_weights(alphas)  # [B,R,No+Nb,1]
        # Compute weights and composite samples.
        rgb = render.composite(rgbs, weights)  # [B,R,3]
        if dnormal_dws != None:
            dnormal_dw = render.composite(dnormal_dws, weights)
            # print(f'dnormal dw {dnormal_dw.min()} <-> {dnormal_dw.max()}')
        else:
            dnormal_dw = None
        if self.white_background:
            opacity_all = render.composite(1., weights)  # [B,R,1]
        ...
        output = dict(
            ...
            dnormal_dw=dnormal_dw, #[B,R,3*256]
            dnormal_dws=output_object["dnormal_dw"], #[B,R,No,3*256]
    ...
    def render_rays_object(self, center, ray_unit, near, far, outside, app, stratified=False):
        ...
        gradients, hessians, dnormal_dw = self.neural_sdf.compute_gradients(points, training=self.training, sdf=sdfs, with_dn_dw=self.use_jacobi and self.training)
        if dnormal_dw != None:
            dnormal_dw = dnormal_dw.reshape(*sdfs.shape[:-1], -1) # [B,R,N,3*hidden]
            # print('Dnormal_dw = ', dnormal_dw.shape)
            # print('Dnormal dw : ', dnormal_dw[0, :3, :3, :5])
        ...
        output = dict(
            ...
            dnormal_dw=dnormal_dw, # [B,R,N,3*hidden_dim] / None
        )
        return output
