...
class NeuralSDF(torch.nn.Module):
    ...
    def __init__(self, cfg_sdf):
        ...
        self.use_jacobi = self.cfg_sdf.use_jacobi
    ...
    def build_mlp(self, cfg_mlp, input_dim=3):
        ...
        self.mlp = MLPforNeuralSDF(layer_dims, skip_connection=cfg_mlp.skip, activ=activ,
                                   ...
                                   out_bias=cfg_mlp.out_bias, invert=cfg_mlp.inside_out, use_jacobi=self.use_jacobi)
    ...
    def compute_gradients(self, x, training=False, sdf=None, with_dn_dw=False):
        ...
        if self.cfg_sdf.gradient.mode == "analytical":
            ...
            if self.cfg_sdf.gradient.taps == 6:
                ...
                if training:
                    ...
                    hessian = None
                if with_dn_dw:
                    assert self.use_jacobi, "only jacobi can use this"
                    dnormal_dw_list = []
                    gradient_shape = gradient.shape
                    # print('Gradient Shape : ', gradient.shape)
                    # print('Gradient Sample : ', gradient[0, 0, 0, :])
                    for i in range(1):
                    # Attention!: use numerical degenerate the dnormal/dw related to the output of the last hidden layer.
                    # So we only need to calc onces
                    # for i in range(gradient.shape[-1]):
                        normal_now = gradient[..., i:i+1]
                        # print('normal now Shape : ', normal_now.shape)
                        d_output = torch.ones_like(normal_now, requires_grad=False, device=gradient.device)
                        dnormal_dw = torch.autograd.grad(
                            outputs=normal_now,
                            inputs=self.mlp.linear_sdf.weight_matrix,
                            grad_outputs=d_output,
                            create_graph=True,
                        )[0]
                        # print('One Type : ', dnormal_dw.shape)
                        dnormal_dw_list.append(dnormal_dw.clone())
                        
                    dnormal_dw_total = torch.stack(dnormal_dw_list, dim=1)
                    dnormal_dw_total = dnormal_dw_total.reshape(*gradient_shape[:2], -1)
                    # print('Total Type : ', dnormal_dw_total.shape)
                    # print('Peak Gradient : ', dnormal_dw_total[0, :5, :5])
            else:
                raise ValueError("Only support 4 or 6 taps.")
        if with_dn_dw:
            return gradient, hessian, dnormal_dw_total
        else:
            return gradient, hessian, None
