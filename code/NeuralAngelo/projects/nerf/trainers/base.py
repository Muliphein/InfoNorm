...
class BaseTrainer(BaseTrainer):
    ...
    @staticmethod
    def calc_psnr_picture(pred_image, gt_image):
        import torch.nn.functional as torch_F
        psnr = -10 * torch_F.mse_loss(pred_image, gt_image).log10()
        return psnr 

    
    @torch.no_grad()
    def calc_psnr(self, data_loader, output_dir=None, inference_args=None, mode="test", show_pbar=False, post_fix="None"):
        ...
        for it, data in enumerate(data_loader):
            ...
            output = model.inference_lite(data)
            image = data['image']
            rgb = output['rgb_map']
            psnr = self.calc_psnr_picture(rgb, image).cpu()
            # print(f'PSNR : {psnr}')
            psnr_list .append(psnr)
            if it == 0:
                save_path = f"eval_sample_{post_fix}.jpg"
                torchvision.utils.save_image(rgb, save_path)
                save_path = f"eval_sample_gt_{post_fix}.jpg"
                torchvision.utils.save_image(image, save_path)
        # print(psnr_list)
        total = sum(psnr_list)
        mean = total / len(psnr_list)
        print(f'{post_fix} PSNR : {mean}')
        if post_fix == "Train":
            psnr_sub_list = [psnr_list[i] for i in self.eval_data_subset]
            total = sum(psnr_sub_list)
            mean = total / len(psnr_sub_list)
            print(f'{post_fix}Inlier PSNR : {mean}')
