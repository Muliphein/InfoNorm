...
class BaseTrainer(object):
    ...
    def set_data_loader(self, cfg, split, shuffle=True, drop_last=True, seed=0):
        ...
        if split == "train":
            ...
            self.test_data_loader = get_test_dataloader(cfg)
    ...
    def train_step(self, data, last_iter_in_epoch=False):
        ...
        # self.tbar.set_postfix_str(f'Total Loss {total_loss.detach()}')
        # print(f'Gradient Before : ', self.model.module.neural_sdf.mlp.linear_sdf.weight.grad)
        self.scaler.scale(total_loss).backward()
        # print(f'Gradient After : ', self.model.module.neural_sdf.mlp.linear_sdf.weight.grad)
    ...
    def train(self, cfg, data_loader, single_gpu=False, profile=False, show_pbar=False):
        ...
        self.timer.reset_timeout_counter()
        tbar = tqdm(
            total=min((cfg.max_epoch-start_epoch), (cfg.max_iter - start_epoch * len(data_loader) + len(data_loader) -1 )//len(data_loader))
        )
        # self.tbar = tbar
    ...
    def _get_total_loss(self):
        ...
        for loss_name in self.weights:
            ...
            if loss_name in self.losses:
                ...
                # print(f"Now Loss : {total_loss}, contains :{loss_name}")
