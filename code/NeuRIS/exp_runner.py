...
from models.utils import mesh_calc, psnr_calc, clean_mesh, convert_mesh
...
class Runner:
    def __init__(self, conf_path, mode='train', case = 'CASE_NAME', split='SPLIT_NAME', is_continue=False):
        ...
        conf_text = conf_text.replace('SPLIT_NAME', split)
        f.close()

        self.conf = ConfigFactory.parse_string(conf_text)
        self.conf['dataset.data_dir'] = self.conf['general.data_dir']
        
        self.conf['dataset']['jacobi_start'] = self.conf['model.loss.jacobi_start']
        self.conf['dataset']['jacobi_end'] = self.conf['model.loss.jacobi_end']

        self.base_exp_dir = self.conf['general.base_exp_dir']
        self.dataset_type = self.conf['general.dataset_type']
        try:
            print(f"Dataset Setting jacobi pos threshold = {self.conf['dataset']['jacobi_pos_threshold']}")
        except:
            self.conf['dataset']['jacobi_pos_threshold'] = self.conf['model']['loss']['jacobi_pos_threshold']
        self.scan_name = case
    ...
    def build_dataset(self):
        ...
        self.conf['dataset']['data_dir']  = os.path.join(self.conf['general.data_dir'])
        self.dataset = Dataset(self.conf['dataset'], mode=self.mode)
    ...
    def get_near_far(self, rays_o, rays_d,  image_perm = None, iter_i= None, pixels_x = None, pixels_y= None):
        ...
        # print('Near Far = ', near.min(), far.max())
    ...
    def get_model_input(self, image_perm, iter_i):
        ...
        data, pixels_x, pixels_y,  normal_sample, planes_sample, subplanes_sample, jaco_info, jaco_mask = self.dataset.random_get_rays_at(idx_img, self.batch_size)
        
        # print(f'Jaco Info Shape : {jaco_info.shape}')
        ...
        input_model['jaco_info'] = jaco_info
    ...
    def render_view(self, idx=None, pose=None, intr_inv = None, resolution_level=1):
        
        if idx is not None:
            pose = self.dataset.pose_all[idx]
            intr_inv = self.dataset.intrinsics_all_inv[idx]
            rays_o, rays_d = self.dataset.gen_rays_at_pose(pose=pose, intr_inv=intr_inv, resolution_level=resolution_level)
        else:
            assert pose is not None and intr_inv is not None
            rays_o, rays_d = self.dataset.gen_rays_at_pose(pose=pose, intr_inv=intr_inv, resolution_level=resolution_level)

        imgs_render = {}
        for key in ['color_fine', 'confidence', 'normal', 'depth', 'variance_surface', 'confidence_mask', 'feature']:
            imgs_render[key] = []
        ...
        for key in imgs_render:
            if len(imgs_render[key]) > 0:
                ...
                print(f'Pic {key} reshape to {H}x{W}', end='')
                imgs_render[key] = imgs_render[key].reshape(H, W, -1)
                print(f' = {imgs_render[key].shape}')

        return imgs_render

    def export_feat(self, idx=-1, resolution_level=-1):
        assert idx != -1 and resolution_level != -1
        pose = self.dataset.pose_all[idx]
        intr_inv = self.dataset.intrinsics_all_inv[idx]
        render_result = self.render_view(
            idx=idx,
            pose=pose,
            intr_inv=intr_inv,
            resolution_level=resolution_level
        )
        feature_result = render_result['feature']
        print(f'Feat Shape : {feature_result.shape}')
        os.makedirs(os.path.join(self.base_exp_dir, 'jaco_fine'), exist_ok=True)
        np.save(os.path.join(self.base_exp_dir,
                                'jaco_fine',
                                f'{self.dataset.names_lis[idx]}.npy'),
                                feature_result)

    def generate_all_views(self, ):
        os.makedirs(os.path.join(self.base_exp_dir, 'trainview'), exist_ok=True)
        os.makedirs(os.path.join(self.base_exp_dir, 'evalview'), exist_ok=True)
        tbar = tqdm(total=len(self.dataset.names_lis) + len(self.dataset.eval_names_lis))
        skip = len(self.dataset.names_lis) + len(self.dataset.eval_names_lis) // 20

        for idx, name in enumerate(self.dataset.names_lis[::skip]):
            pose = self.dataset.pose_all[idx]
            intr_inv = self.dataset.intrinsics_all_inv[idx]
            render_result = self.render_view(pose=pose, intr_inv=intr_inv, resolution_level=1)
            img_result = render_result['color_fine']
            print(f'Image Range : [{img_result.min()},{img_result.max()}]')
            ImageUtils.write_image(
                ...
                (img_result * 256).clip(0, 255)
            )
            tbar.update(1)
            
        for idx, name in enumerate(self.dataset.eval_names_lis[::skip]):
            pose = self.dataset.eval_pose_all[idx]
            intr_inv = self.dataset.eval_intrinsics_all_inv[idx]
            render_result = self.render_view(pose=pose, intr_inv=intr_inv, resolution_level=1)
            img_result = render_result['color_fine']
            ImageUtils.write_image(
                os.path.join(
    ...
    def make_mesh_metrics(self,):
        # Validate Mesh
        pred_mesh_name = os.path.join(self.base_exp_dir, f'result.ply')
        
        self.validate_mesh(
            world_space=False,
            resolution=512,
            threshold=0.0,
            path_mesh=pred_mesh_name,
        )

        # Mesh Regenerate
        
        if os.path.exists(os.path.join(self.dataset.data_dir, 'trans_n2w.txt')):
            convert_mesh(
                input_mesh = os.path.join(self.dataset.data_dir, 'mesh.ply'),
                output_mesh = os.path.join(self.base_exp_dir, 'gt-mesh.ply'),
                trans_file = os.path.join(self.dataset.data_dir, 'trans_n2w.txt'),
                inv=True
            )
        else:
            convert_mesh(
        ...
        convert_mesh(
            input_mesh = os.path.join(self.base_exp_dir, 'gt-mesh.ply'),
            output_mesh = os.path.join(self.base_exp_dir, 'gt-mesh.ply'),
            scale_mat=self.dataset.scale_mats_np[0],
            inv=False
        )
        ...
        convert_mesh(
            input_mesh = os.path.join(self.base_exp_dir, 'result.ply'),
            output_mesh = os.path.join(self.base_exp_dir, 'result.ply'),
            scale_mat=self.dataset.scale_mats_np[0],
            inv=False
        )
        ...
        clean_mesh(
            input_mesh=os.path.join(self.base_exp_dir, 'result.ply'),
            mid_mesh=os.path.join(self.base_exp_dir, 'result-clean.ply'),
            output_mesh=os.path.join(self.base_exp_dir, 'result-final.ply'),
            cam_file=os.path.join(self.dataset.data_dir, self.dataset.cameras_name),
            name_lis=self.dataset.names_lis,
            w = self.dataset.W,
            h = self.dataset.H,
            extract_length = 512,
        )
        clean_mesh(
            input_mesh=os.path.join(self.base_exp_dir, 'gt-mesh.ply'),
            mid_mesh=os.path.join(self.base_exp_dir, 'gt-mesh-clean.ply'),
            output_mesh=os.path.join(self.base_exp_dir, 'gt-mesh-final.ply'),
            cam_file=os.path.join(self.dataset.data_dir, self.dataset.cameras_name),
            name_lis=self.dataset.names_lis,
            w = self.dataset.W,
            h = self.dataset.H,
        ...
        mesh_calc(
            mesh_gt=os.path.join(self.base_exp_dir, 'gt-mesh-final.ply'),
            mesh_pred=os.path.join(self.base_exp_dir, 'result-final.ply'),
            thres_list=[0.001, 0.002, 0.005, 0.008, 0.01, 0.02, 0.03, 0.05],
            scale = self.dataset.scale_mats_np[0][0][0],
            save_file=os.path.join(self.base_exp_dir, 'MESH.csv')
        )

    def make_psnr_metrics(self,):
        # Validate Views
        self.generate_all_views()

        print('Starting Metrics Calc')
        
        skip = len(self.dataset.names_lis) + len(self.dataset.eval_names_lis) // 20
        # Calc only 20 pictures
        
        #* Train View
        train_psnr = psnr_calc(
            list_gt = [os.path.join(self.dataset.data_dir, 'image', i+'.png') for i in self.dataset.names_lis[::skip]],
            list_pred = [os.path.join(self.base_exp_dir, 'trainview', i+'.png') for i in self.dataset.names_lis[::skip]]
        )
        print('Train PSNR : ', sum(train_psnr)/len(train_psnr))

        #* Eval View
        eval_psnr = psnr_calc(
            list_gt = [os.path.join(self.dataset.data_dir, 'image', i+'.png') for i in self.dataset.eval_names_lis[::skip]],
            list_pred = [os.path.join(self.base_exp_dir, 'evalview', i+'.png') for i in self.dataset.eval_names_lis[::skip]]
        )
        print('Eval PSNR : ', sum(eval_psnr)/len(eval_psnr))
        
        csv_file = os.path.join(self.base_exp_dir, 'PSNR.csv')
        import csv
        with open(csv_file, 'a', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(['Train PSNR', 'Valid PSNR'])
            writer.writerow([np.mean(train_psnr), np.mean(eval_psnr)])
        

    def make_metrics(self,):
        self.make_psnr_metrics()
        self.make_mesh_metrics()
    ...
    def validate(self, input_model, loss_out, render_out):
        ...
        if self.iter_step % self.report_freq == 0:
            ...
            logging.info(
                'loss_total={:.03f} lr={:.06f} var={:.04f} rgb={:.04f}, Eik={:.04f}, Normal={:.04f}, jaco={:.04f}, jaco_gra={:.04f}, jaco_con={:.04f}'.format(
                    loss, self.optimizer.param_groups[0]['lr'], render_out['variance'].mean(),
                    loss_out['Loss/loss_color'],
                    loss_out['Loss/final_eikonal'],
                    loss_out['Loss/final_normal'],
                    loss_out['Loss/jacobi_loss'],
                    loss_out['Log/jacobi_gradient_loss'],
                    loss_out['Log/jacobi_contrastive_loss'],
                )
            )
    ...
    def validate_mesh(self, world_space=False, resolution=128, threshold=0.0, path_mesh=None):
        ...
        if path_mesh is None:
            path_mesh = os.path.join(self.base_exp_dir, 'meshes', f'{self.iter_step:0>8d}.ply')
        if world_space:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--case', type=str, default='', help='Scene or scan name')
    ...
    if args.mode == 'train':
        runner.train()
    elif args.mode == 'make_metrics':
        runner.make_metrics()
    elif args.mode == 'make_mesh_metrics':
        runner.make_mesh_metrics()
    elif args.mode == 'export_feat_sample':
        runner.export_feat(
            idx=0,
            resolution_level=1
        )
