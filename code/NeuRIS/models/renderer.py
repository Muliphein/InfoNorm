...
class NeuSRenderer:
    def __init__(self,
        ...
        self.radius = 1.0
        self.use_dn_dp = use_dn_dp
        self.feature_from = feature_from
        self.is_sdf_dn_dp = is_sdf_dn_dp
        self.is_c_dn_dp = is_c_dn_dp
        self.normal_gather = normal_gather
        if self.normal_gather:
            print('[Notice] normal gather is open')
    ...
    def up_sample(self, rays_o, rays_d, z_vals, sdf, n_importance, inv_variance):
        ...
        prev_cdf = torch.sigmoid(prev_esti_sdf * inv_variance) # When inv_variance ↑ more points generated in zero surface
    ...
    def render_core(self,
        ...
        if self.use_dn_dp and not self.is_c_dn_dp and not self.normal_gather:
            if self.is_sdf_dn_dp:
                gradients = sdf_network.gradient(pts)
                dn_dp_feature = sdf_network.dsdf_dw(pts)
                # print('Feature from SDF :', dn_dp_feature.shape)
            else:
                dn_dp_feature, gradients = sdf_network.dnormal_dw(pts)
                # print('Feature from Normal :', dn_dp_feature.shape)
        else:
            gradients = sdf_network.gradient(pts)
        
        if self.use_dn_dp and self.is_c_dn_dp and not self.normal_gather:
            sampled_color, _ = color_network(pts, gradients, dirs, feature_vector)
            _, dn_dp_feature, sampled_feature = color_network.dcolor_dw(pts, gradients, dirs, feature_vector)
            # print('Feature from Color :', dn_dp_feature.shape)
        else:
            sampled_color, sampled_feature = color_network(pts, gradients, dirs, feature_vector)

        if self.normal_gather:
            sampled_feature = gradients
        elif self.feature_from == 'rendering':
            print('No This')
            assert False
            pass
        elif self.feature_from == 'sdf':
            # print('Use SDF Feature')
            sampled_feature = dn_dp_feature
        else:
            raise NotImplementedError(f"No Such Feature {self.feature_from}")

        sampled_color = sampled_color.reshape(batch_size, n_samples, 3)
        if sampled_feature is not None:
            sampled_feature_shape = sampled_feature.squeeze().shape
            # print(f'Feature Before : {sampled_feature_shape}')
            if len(sampled_feature_shape) == 2:
                sampled_feature = sampled_feature.reshape(batch_size, n_samples, -1) #* [bs, n, 256] for sum
            elif len(sampled_feature_shape) == 3:
                sampled_feature = sampled_feature.reshape(batch_size, n_samples, sampled_feature_shape[1], sampled_feature_shape[2]) #* [bs, n, 3, 256] for single
            else: raise NotImplementedError
            # print(f'Feature After : {sampled_feature.shape}')
        ...
        if sampled_feature is not None:     # Calculate Feature only when feature not None
            if len(sampled_feature_shape) == 2:
                feature = (sampled_feature * weights[:, :, None]).sum(dim=1)
            elif len(sampled_feature_shape) == 3:
                feature = (sampled_feature * weights[:, :, None, None]).sum(dim=1)
            else: raise NotImplementedError
        else:
            feature = None
        ...
        if self.n_importance > 0:
            with torch.no_grad():
                ...
                for i in range(n_steps):
                    ...
                    # Upsample + n_importance // n_steps points
