...
class ReconstructionTrainer(pl.LightningModule):
    def __init__(self, conf, prog_bar: RichProgressBar, exp_dir, model_only=False, val_mesh=False, is_val=False, **kwargs) -> None:
        ...
        use_normal = (getattr(conf.loss, 'normal_weight', 0) > 0) or (getattr(conf.loss, 'angular_weight', 0) > 0) or (getattr(conf.loss, 'jacobi_contrastive_weight', 0) > 0) 
        ...
        self.train_dataset = dataset.ReconDataset(
                **dataset_conf, 
                ...
                use_jaco=getattr(conf.loss, 'jacobi_contrastive_weight', 0) > 0 or getattr(conf.loss, 'jacobi_gradient_weight', 0) > 0,
            )
    ...
    def train_dataloader(self):
        return DataLoader(self.train_dataset, batch_size=self.batch_size, shuffle=True, collate_fn=self.train_dataset.collate_fn, num_workers=0, generator=torch.Generator(device='cuda'),)
    ...
    def training_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            self.log_if_nonzero('train/jac_con', loss_output['jaco_con_loss'].item())
            self.log_if_nonzero('train/jac_gra', loss_output['jaco_gra_loss'].item())
        ...
        # print(f'Batch IDX {batch_idx}, Return loss : ', loss.item())
        return loss
