...
class Runner:
    def __init__(self, conf_path, mode='train', case='CASE_NAME', split='SPLIT_NAME',is_continue=False, checkpoint=False, suffix=''):
        ...
        self.jacobi_contrastive_weight= self.conf.get_float('train.jacobi_contrastive_weight', default=0.0)
        self.jacobi_gradient_weight= self.conf.get_float('train.jacobi_gradient_weight', default=0.0)
    ...
    def train(self):
        ...
        self.validate_mesh_ori(world_space=False, resolution=128, threshold=args.mcube_threshold)

        for iter_i in tqdm(range(res_step)):
            torch.cuda.manual_seed(iter_i)
            torch.manual_seed(iter_i)
            data, intrinsic, intrinsic_inv, pose, image_gray, jaco_info = self.dataset.gen_random_rays_at(image_perm[self.iter_step % len(image_perm)], self.batch_size)
            ...
            if pts_view is not None:
                ...
                pts2sdf = torch.zeros((1, 1), device=color_fine.device).float()
            ...
            if jaco_info is not None and feature is not None:
                jaco_gra_loss, jaco_con_loss = self.get_jacobi_loss(jaco_info, feature)
            else:
                jaco_con_loss = torch.tensor(0.0, device=color_fine.device).float()
                jaco_gra_loss = torch.tensor(0.0, device=color_fine.device).float()


            ncc_loss = 0.5 * (ncc_cost.sum(dim=0) / (inside_sphere.sum(dim=0) + 1e-8)).squeeze(-1)
            # ncc_loss = torch.tensor(0.0, device=color_fine.device).float()
            ...
            loss = color_fine_loss +\
                   ...
                   ncc_loss +\
                   sdf_loss +\
                   jaco_con_loss * self.jacobi_contrastive_weight +\
                   jaco_gra_loss * self.jacobi_gradient_weight
            ...
            self.writer.add_scalar('Loss/jaco_con_loss', jaco_con_loss, self.iter_step)
            ...
            if self.iter_step % self.report_freq == 0:
                ...
                print('iter:{:8>d} loss = {} color_loss={} eikonal_loss={} ncc_loss={} jaco_con={} sdf_loss={} psnr={} lr={}'.format(
                    self.iter_step, loss, color_fine_loss, eikonal_loss,
                    ncc_loss, jaco_con_loss, sdf_loss, psnr, self.optimizer.param_groups[0]['lr']))
        ...
        self.validate_mesh_ori(world_space=True, resolution=512, threshold=args.mcube_threshold, name='result.ply')
    ...
    def validate_mesh_ori(self, world_space=False, resolution=64, threshold=0.0, name=None):
        ...
        if name == None:
            ...
            mesh.export(os.path.join(self.base_exp_dir, name))
...
if __name__ == '__main__':
    ...
    runner = Runner(args.conf, args.mode, args.case, args.split, args.is_continue, args.checkpoint, args.suffix)
