...
def test(model, test_loader, device, results_dir):
    ...
    with torch.no_grad():
        for data_dict in tqdm(test_loader):
            ...
            # 0. save the npz
            np.savez('%s/%s' % (results_dir, img_name), pred_norm)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--architecture', type=str, default="BN", help='{BN, GN}')
    parser.add_argument("--pretrained", type=str, default="scannet", help="{nyu, scannet}")
    parser.add_argument('--sampling_ratio', type=float, default=0.4)
    parser.add_argument('--importance_ratio', type=float, default=0.7)
    parser.add_argument('--input_height', default=360, type=int)
