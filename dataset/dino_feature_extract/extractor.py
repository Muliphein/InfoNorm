...
if __name__ == "__main__":
    ...
    parser.add_argument('--load_size', default=360, type=int, help='load size of the input image.')
    ...
    import os
    os.makedirs(os.path.join(args.output_path, 'dino'), exist_ok=True)
    # print(os.listdir(args.image_path))
    if "rgb" in args.image_path:
        image_list = sorted(glob(os.path.join(args.image_path, "*.png")), key=lambda x: int(os.path.basename(x)[4:-4]))
    else:
    ...
    with torch.no_grad():
        ...
        for image_name in image_list:
            if args.split_file is not None:
                if "rgb" in args.image_path:
                    pic_name = os.path.basename(image_name)[4:-4].zfill(3)+'.png'
                else:
                    pic_name = os.path.basename(image_name)[:-4].zfill(3)+'.png'
                if pic_name not in train_list:
                    continue
            image_batch, image_pil = extractor.preprocess(image_name, args.load_size)
            ...
            print(f"Descriptors are of size: {descriptors.shape}") # [1, 1, 89*119, 384]
            if "rgb" in args.image_path:
                out_path = os.path.join(args.output_path, 'dino', os.path.basename(image_name)[4:-4].zfill(3)+'.pth')
            else:
                out_path = os.path.join(args.output_path, 'dino', os.path.basename(image_name)[:-4]+'.pth')
            torch.save(descriptors, out_path)
            print(f" - {os.path.basename(image_name)} Descriptors saved to: {out_path}")
