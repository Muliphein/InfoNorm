...
def load_colmap_data(realdir):
    ...
    # camerasfile = os.path.join(realdir, 'colmap/cameras.bin')
    # camdata = read_model.read_cameras_binary(camerasfile)
    camerasfile = os.path.join(realdir, 'colmap/cameras.txt')
    ...
    # imagesfile = os.path.join(realdir, 'colmap/images.bin')
    # imdata = read_model.read_images_binary(imagesfile)
    imagesfile = os.path.join(realdir, 'colmap/images.txt')
    ...
    # points3dfile = os.path.join(realdir, 'colmap/points3D.bin')
    # pts3d = read_model.read_points3d_binary(points3dfile)
    points3dfile = os.path.join(realdir, 'colmap/points3D.txt')
...
def gen_poses(basedir, match_type, factors=None):
    ...
    files_needed = ['{}.txt'.format(f) for f in ['cameras', 'images', 'points3D']]
    if os.path.exists(os.path.join(basedir, 'colmap')):
        files_had = os.listdir(os.path.join(basedir, 'colmap'))
    else:
        files_had = []
    if not all([f in files_had for f in files_needed]):
        assert False, "No COLMAP file"
